...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
        ...
        for symbol in symbols {
                ...
                (symbol["symbol"].as_str(), symbol["quoteAsset"].as_str())
            {
                if quote_asset == "BNB" {
                    ...
                    coins.push(format!("BINANCE:{}BNB", base_asset));
                }
            }
        }
    }
    ...
    if coins.is_empty() {
        println!("BNB spot piyasa çiftleri bulunamadı. API yanıtını kontrol edin.");
        return Ok(());
    }

    coins.sort_by(|a, b| {
        let a_name = a.split(':').nth(1).unwrap_or("").trim_end_matches("BNB");
        let b_name = b.split(':').nth(1).unwrap_or("").trim_end_matches("BNB");
    ...
    let mut file = File::create("binance_bnb_markets.txt")?;
    for coin in &coins {
        writeln!(file, "{}", coin)?;
    }

    println!("Binance BNB spot piyasası coin listesi başarıyla oluşturuldu ve kaydedildi.");
    println!("Toplam {} adet BNB spot piyasa çifti bulundu.", coins.len());
